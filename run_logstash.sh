...
export logstashAgentDir=/opt/docker-projects/Docker-ELK-Alfresco-Monitoring-master/logstash-agent
...
if [ "$1" = "start" ] ; then
  ...
  if [ "${collectAuditData}" = "yes" ] ; then
    ...
    nohup touch ${logstashAgentLogs}/audit-access.log && ${logstashAgentDir}/audit-access.sh &>${logstashAgentLogs}/audit-access.log &
  fi
elif [ "$1" = "stop" ] ; then
